...
class Personaje:
    ...
    def atacar(self, otro_personaje):
        critico = random.choice([False] * 3 + [True])  # 25% de probabilidad de crítico
    ...
    def usar_pocion(self):
        if self.pociones > 0:
            ...
            self.hp += curacion
            self.pociones -= 1
            print(f"{self.nombre} usa una poción y recupera {curacion} puntos de vida.")
            print(f"{self.nombre} ahora tiene {self.hp} puntos de vida.\n")
        else:
            print(f"{self.nombre} intenta usar una poción, ¡pero no le quedan!\n")
